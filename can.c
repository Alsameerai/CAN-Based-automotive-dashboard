...
void init_can(void) 
{
     ...
     * Filter 0
     */
    RXFCON0 = 0x00; //filters can accept noly one data so that we are are disabling
    /* Enter CAN module into Loop back mode */
    CAN_SET_OPERATION_MODE_NO_WAIT(e_can_op_mode_normal);//LOOP BACK for tx and Rx in 
...
/*fROM this only we can check if it is from particular msg id -->receive */
/*ECU3*/
...
void can_transmit(uint16_t msg_id, const uint8_t *data, uint8_t len)
{
    ...
    // Set data length
    TXB0DLC = len; //I can send only 2 bytes of data
